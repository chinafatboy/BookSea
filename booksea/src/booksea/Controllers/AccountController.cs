    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    Customer c = db.Customer.Add(new Customer()).Entity; //报错 ，已解决  错误为：对象引用未设置为对象实例。 （没有在构造方法加入）
                    c.UserId = user.Id;
        ...
        //会员中心实现代码包括会员信息修改订单查询

        [Authorize]
        public ActionResult MemberHome(int page = 1,int pageSize = 4)
        {
            ViewBag.pwdDisp = "none";
            string[] orderStates = { "未付款", "已付款", "已排产", "已生产", "已发送", "已签收", "已删除" };
            ...
            int custId = ViewBag.uid = c.Id;
            mhm.CustomerInfo = new RegisterViewModel { UserName = c.UserName, Email = c.Email, MobilePhone = c.MobilePhone, OfficePhone = c.OfficePhone, HomePhone = c.HomePhone, QqNum = c.QqNumber };
            var orderlist = from a in db.TheOrder
                            where a.OrderState < 6 && a.Customer == custId
                            join b in db.Book on a.TheBook equals b.Id
                            join p in db.Payment on a.ThePayment equals p.Id
                            join d in db.Consigenn on a.TheConsignee equals d.Id
                            join e in db.CustomerWords on a.Id equals e.TheOrder
           ...
           // var orders = orderlist.Skip(page -1); 
        ...
        public async void updateMemberInfo(int memberId, string memberMobile, string memberOffice, string memberHome, string memberQQ)
        {
            ...
            Customer c = db.Customer.Single(m => m.Id == memberId);
